...
from . import spatial
from . import Encoder
from .hdgenerator import HDGenerator
...
class OnlineHD(object):
    ...
    def __init__(self, classes : int, features : int, dim : int = 4000):
        ...
        self.generator = HDGenerator(features, dim)
    ...
    def encode(self, x : torch.Tensor):
        ...
        return self.encoder(x, self.basis, self.base, self.alpha)


    def generate(self, size : int, target : str):
        return self.generator(size, target, self.basis, self.base, self.alpha)
    ...
    def fit(self,
        ...
        self.alpha = (torch.matmul(x, self.basis.T).max()+self.base).max().item() / math.pi + 0.0001
        
        h = x if encoded else self.encode(x)
        self.h = h
        
        self.generator.fit(h, y)
        if one_pass_fit:
